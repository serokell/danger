...
RSpec.describe "trailing-whitespace check" do
  ...
  it "suggests removing exactly the extra newlines" do
    # The fixture has a single extra blank line at the end (8 lines total),
    # so the suggested diff should be empty (removing that one line) and
    # anchored at line 8.
    simple_test(
      trailing_whitespace_extra_final_newlines_path,
      {extra_final_newlines: {}}
    ) do
      expect(dangerfile).to receive(:markdown).with(
        "Extra newline at the end of the file.\n```suggestion\n```\n",
        file: trailing_whitespace_extra_final_newlines_path, line: 8
      )
    end
  end
