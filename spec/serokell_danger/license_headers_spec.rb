...
RSpec.describe "license-headers check" do
  ...
  # This check only validates the year/holder on a header that's already
  # there; it's not meant to catch a missing header entirely. That's
  # reuse lint's job (REUSE/SPDX compliance), so a file with no
  # header-shaped line at all is silently skipped here.
