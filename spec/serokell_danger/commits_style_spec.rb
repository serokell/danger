...
RSpec.describe "commits-style check" do
  ...
  it "does not warn about a commit with a valid YouTrack-style issue prefix" do
    simple_test(make_commit(subject: "[SRK-160] Issue prefix"), {})
  end

  it "warns about a lowercase subject after a valid YouTrack-style issue prefix" do
    # Regression case: the prefix itself contains an uppercase letter
    # (the "S" in "SRK"), so this proves subject_starts_with_uppercase
    # checks the payload after the full prefix is stripped, not just
    # whether an uppercase letter appears anywhere near the start.
    simple_test(
      make_commit(subject: "[SRK-160] issue prefix"),
      {subject_starts_with_uppercase: {}}
    )
  end
