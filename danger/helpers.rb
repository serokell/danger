...
module SerokellDanger
  DRAFT_TITLE_PATTERN = /\A(?:\[Draft\]\s*|(?:Draft|WIP)\s*:\s*)/i.freeze

  ANY_LEADING_TAGS_PATTERN = /\A(?:\[[^\]]*\]\s*)*/.freeze
  ...
  class GitHost
    ...
    alias pr_title mr_title

    def mr_body
      plugin.mr_body.to_s
    end
    alias pr_body mr_body

    def mr_json
      plugin.mr_json
    end
    alias pr_json mr_json

    def branch_for_head
      plugin.branch_for_head
    end
    alias source_branch branch_for_head

    def branch_for_base
      plugin.branch_for_base
    end
    alias target_branch branch_for_base
    ...
    alias pr_draft? mr_draft?

    def mr_title_payload
      mr_title.sub(DRAFT_TITLE_PATTERN, '')
    end
    alias pr_title_payload mr_title_payload
...
class Git::Object::Commit
  def subject
    self.message.lines.first.to_s.rstrip
  end
